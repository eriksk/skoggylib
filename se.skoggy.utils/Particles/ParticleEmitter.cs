...
namespace se.skoggy.utils.Particles
{
    public class ParticleEmitter
    {
        ...
        public void Draw(Camera cam, SpriteBatch spriteBatch, Rectangle[] sources, GameObject template) 
        {
            spriteBatch.Begin(SpriteSortMode.Immediate, settings.blendState, null, null, null, null, cam.Projection);
            for (int i = 0; i < particles.Count; i++)
            {
                Particle p = particles[i];
                template.position.X = p.position.X;
                template.position.Y = p.position.Y;
